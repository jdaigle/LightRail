...
namespace LightRail.ServiceBus.Reflection
{
    ...
    /// </summary>
    public class ReflectionMessageMapper : IMessageMapper
    {
        public void Initialize(IEnumerable<Type> types)
        {
            ...
            {
                if (moduleBuilderInstance == null)
                {
                    var dynamicAssemblyName = types.First().Namespace + SUFFIX;
                    var assemblyBuilder = AppDomain.CurrentDomain.DefineDynamicAssembly(
                        new AssemblyName(dynamicAssemblyName),
                        AssemblyBuilderAccess.Run
                        );
                    moduleBuilderInstance = assemblyBuilder.DefineDynamicModule(dynamicAssemblyName);
                }
                foreach (var t in types)
                {
                    InitType(t, moduleBuilderInstance);
                }
            }
        }
        ...
        /// </summary>
        private static void InitType(Type t, ModuleBuilder moduleBuilder)
        {
            if (t == null)
                return;
        ...
        private static void GenerateImplementationFor(Type interfaceType, ModuleBuilder moduleBuilder)
        {
            if (interfaceType.GetMethods().Any(mi => !(mi.IsSpecialName && (mi.Name.StartsWith("set_") || mi.Name.StartsWith("get_")))))
            {
                Logger.Warn("Interface {0} contains methods and can there for not be mapped. Be aware that non mapped interface can't be used to send messages.", interfaceType.Name);
                return;
            }
        ...
        /// </summary>
        private static Type CreateTypeFrom(Type t, ModuleBuilder moduleBuilder)
        {
        ...
        public object CreateInstance(Type t)
        {
            ...
            if (t.IsInterface || t.IsAbstract)
            {
                ...
                if (mapped == null)
                {
                    Initialize(new[] { t }); // lazy initialize the type
                    mapped = GetMappedTypeFor(t);
                    if (mapped == null)
                    {
        ...
        private static ILogger Logger = LogManager.GetLogger("LightRail.ServiceBus.Reflection.ReflectionMessageMapper");
        private static ModuleBuilder moduleBuilderInstance;
        ...
        public List<Type> GetMessageTypeHierarchy(Type type)
        {
            ...
            {
                if (!types.Contains(t) && !IsSystemType(t))
                {
                    types.Add(t);
                }
            }
            var baseType = type.BaseType;
            while (baseType != null && baseType != typeof(object) && !IsSystemType(baseType))
            {
        ...
        {
            // TODO cache this operation
            return GetMessageTypeHierarchy(type).Select(x => x.FullName);
        }
    }
}