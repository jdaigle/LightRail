...
namespace LightRail.SqlServer
{
    public static class ServiceBrokerWrapper
    {
        ...
        public static ServiceBrokerMessage Receive(IDbTransaction transaction, string queueName)
        {
            return ReceiveInternal(transaction, queueName, null, false, null);
        }

        public static ServiceBrokerMessage Receive(IDbTransaction transaction, string queueName, Guid conversationHandle)
        {
            return ReceiveInternal(transaction, queueName, conversationHandle, false, null);
        }

        public static ServiceBrokerMessage WaitAndReceive(IDbTransaction transaction, string queueName, int waitTimeout)
        {
            return ReceiveInternal(transaction, queueName, null, true, waitTimeout);
        }

        public static ServiceBrokerMessage WaitAndReceive(IDbTransaction transaction, string queueName, Guid conversationHandle, int waitTimeout)
        {
        ...
        public static void CreateServiceAndQueue(IDbTransaction transaction, string serviceName, string queueName, string messageTypeName, string messageContractName)
        {
            ...
            query.AppendFormat("IF NOT EXISTS (SELECT * FROM sys.service_message_types WHERE name = '{0}')" + Environment.NewLine, messageTypeName);
            query.AppendFormat("CREATE MESSAGE TYPE [{0}] AUTHORIZATION dbo VALIDATION = NONE;" + Environment.NewLine, messageTypeName);
            query.AppendFormat("IF NOT EXISTS (SELECT * FROM sys.service_contracts WHERE name = '{0}')" + Environment.NewLine, messageContractName);
            query.AppendFormat("CREATE CONTRACT [{0}] AUTHORIZATION dbo ([{1}] SENT BY INITIATOR);" + Environment.NewLine, messageContractName, messageTypeName);
            ...
            query.AppendFormat("CREATE SERVICE [{0}]  AUTHORIZATION [dbo] ON QUEUE [dbo].[{1}] ([{2}]);" + Environment.NewLine, serviceName, queueName, messageContractName);
